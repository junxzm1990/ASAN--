...
bool btraceInLoop(Value *v, std::vector<Value *> &backs, Loop *L) {
  ...
  if(ConstantExpr *i_cexp = dyn_cast<ConstantExpr>(v)){
    ...
    if(i_call->isInlineAsm()){
      InlineAsm *bin = dyn_cast<InlineAsm>(i_call->getCalledOperand());
      assert(bin != nullptr);
...
bool btraceIndex(Value *v, std::vector<Value *> &backs, Instruction *DefineInst) {
  ...
  // hit the define to stop
  if (Value *DefIndex = DefineInst->getOperand(0)) {

    if(std::find(backs.begin(), backs.end(), DefIndex) != backs.end()){
      return false;
    }
  ...
  if(ConstantExpr *i_cexp = dyn_cast<ConstantExpr>(v)){
    ...
    User::op_iterator oi = i_cexp->op_begin(), oe = i_cexp->op_end();
    for(; oi != oe; ++oi){
      if(btraceIndex(*oi, backs, DefineInst)){
        res = true;
      }
    }
  }
    ...
    // only trace the condition value
    if(i_brch->isConditional()){
      if(btraceIndex(i_brch->getCondition(), backs, DefineInst)){
        res = true;
      }
    }
  }
    ...
    // If there is only the default destination, just branch.
    if (!i_swch->getNumCases()) {
      if(btraceIndex(i_swch->getDefaultDest(), backs, DefineInst)){
        res = true;
      }
    } 
    else {
      // else only trace the condition value
      if(btraceIndex(i_swch->getCondition(), backs, DefineInst)){
        res = true;
      }
    }
  }
...
// #undef LIBCALL_BACK
    ...
    if(btraceIndex(i_bin->getOperand(0), backs, DefineInst)){
      res = true;
    }

    if(btraceIndex(i_bin->getOperand(1), backs, DefineInst)){
      res = true;
    }
  }

  else if(CastInst *i_cast = dyn_cast<CastInst>(v)){
    if(btraceIndex(i_cast->getOperand(0), backs, DefineInst)){
      res = true;
    }
  }
    ...
    if(btraceIndex(i_cmp->getOperand(0), backs, DefineInst)){
      res = true;
    }
    
    if(btraceIndex(i_cmp->getOperand(1), backs, DefineInst)){
      res = true;
    }
  }

  else if(ExtractValueInst *i_ext = dyn_cast<ExtractValueInst>(v)){
    if(btraceIndex(i_ext->getAggregateOperand(), backs, DefineInst)){
      res = true;
    }
  }

  else if(GetElementPtrInst *i_gep = dyn_cast<GetElementPtrInst>(v)){
    if(btraceIndex(i_gep->getPointerOperand(), backs, DefineInst)){
      res = true;
    }
    ...
    if(btraceIndex(i_load->getPointerOperand(), backs, DefineInst)){
      res = true;
    }
    //get all the users of the pointer operand
    //if a user is a store, inlude the store instruction into the slicing process
    for (User *U : i_load->getPointerOperand()->users()) {
      if (StoreInst *i_store = dyn_cast<StoreInst>(U)) {
        if(btraceIndex(U, backs, DefineInst)){
          res = true;
        }
      }
    }
    ...
    unsigned num = i_phi->getNumIncomingValues();
    for(int i = 0; i < num; i++){
      if(btraceIndex(i_phi->getIncomingValue(i), backs, DefineInst)){
        res = true;
      }
      ...
      if(btraceIndex(pt, backs, DefineInst)){
        res = true;
      }
    }
  }
    ...
    if(btraceIndex(i_sel->getTrueValue(), backs, DefineInst)){
      res = true;
    }

    if(btraceIndex(i_sel->getFalseValue(), backs, DefineInst)){
      res = true;
    }

    if(btraceIndex(i_sel->getCondition(), backs, DefineInst)){
      res = true;
    }
  }
    ...
    LLVMContext& C = (*i_store).getContext();
    MDNode* N = MDNode::get(C, MDString::get(C, "Backward Slicing STORE Inst"));
    (*i_store).setMetadata("ASAN.Backward_Slicing_STORE_Inst", N);

    if(btraceIndex(i_store->getValueOperand(), backs, DefineInst)){
      res = true;
    }

    if(btraceIndex(i_store->getPointerOperand(), backs, DefineInst)){
      res = true;
    }
  }

  else if(InsertValueInst *i_insert = dyn_cast<InsertValueInst>(v)){
    if(btraceIndex(i_insert->getAggregateOperand(), backs, DefineInst)){
        res = true;
    }

    if(btraceIndex(i_insert->getInsertedValueOperand(), backs, DefineInst)){
        res = true;
    }
  }
